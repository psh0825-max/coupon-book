...
- ~~(1)은 이미 충족 — vc4·vc5 모두 targetSdk 36. (2)가 미충족~~
  **[2026-08-11 정정]** 실제 원인은 프로덕션이 아니라 **내부 테스트 트랙에 남아
  있던 vc3(1.0.1, targetSdk 35)**였음 — 정책 상태의 "검토된 App Bundle"에 vc3만
  표시됨. 내부 테스트 트랙에 vc4(1.0.2, targetSdk 36)를 라이브러리에서 추가해
  게시함으로써 해소(2026-08-11). Alpha 트랙·14일 카운트는 건드리지 않음.
- 교훈: 이 경고는 **모든 활성 트랙**의 번들을 검사한다. 오래된 트랙에 남은
  저타겟 번들도 경고를 유발하므로, 정책 상태 → 세부정보 → "App Bundle 보기"로
  정확한 대상 번들을 확인할 것.
