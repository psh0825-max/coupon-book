...
- 광고 슬롯: 무료 버전 광고 영역(플레이스홀더). `static/js/ads.js`에서 AdSense ID 넣고 켜면 실제 광고
- 데이터 영속화: IndexedDB (완전 로컬), JSON 백업/복원
- PWA 지원 (오프라인 캐싱, 설치형)
...
│   │   ├── skins.js         # 스킨/카테고리 데이터 + 선택기
│   │   ├── ui.js            # UI 컴포넌트 렌더러(카드, 스탬프, 토스트, 모달, 광고배너)
│   │   ├── location.js      # Geolocation + Haversine + 체류 알림
│   │   ├── fx.js            # 햅틱 + 컨페티 마이크로인터랙션
│   │   ├── ads.js           # 광고 슬롯(AdSense) 연동 스캐폴딩
│   │   └── app.js           # 라우터, 페이지 렌더러, 이벤트 핸들러
...
export const AD_CONFIG = {
  ...
  slot: 'XXXXXXXXXX'                   // 광고 단위 슬롯 ID
};
```

비활성 상태에서는 중립 플레이스홀더 배너가 표시됩니다.
